    ...
    methods: {
      ...
      handOver(e) {
        if (!document.getElementById('_magnifier_layer')) {
          ...
          imgLayer.style.top = -1+ 'px';
          imgLayer.style.backgroundImage = `url('${this.bigsrc}')`;
          imgLayer.style.backgroundRepeat = 'no-repeat';
          if (this.configs.id) {
          	document.querySelector(this.configs.id).appendChild(imgLayer);
          }else{
...
<style>
  #_magnifier_layer{position: absolute; z-index: 9999; background: #f9f9f9; border: 1px solid #eee;}
